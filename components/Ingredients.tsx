    ...
    <div id="ingredientes" ref={reveal.ref} className="relative py-12 px-4 md:px-0">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative z-10">
        {items.map((item, idx) => (
          ...
          >
            <div className="mb-8 text-[#E3C08D] flex justify-center md:justify-start">
              <div className="p-4 bg-[#FCF9F4] rounded-full group-hover:bg-[#E3C08D] group-hover:text-white transition-colors duration-500">
                {item.icon}
              </div>
            </div>

            <div className="mb-6 text-center md:text-left">
              <span className="text-[11px] font-bold uppercase tracking-[0.2em] text-[#E3C08D] mb-2 block">
                {item.title}
              </span>
            ...
            <p className="text-gray-500 text-sm md:text-[15px] leading-relaxed font-light flex-grow text-center md:text-left" dangerouslySetInnerHTML={{ __html: item.description }}>
            </p>
          </div>
        ))}
      </div>
      <div className="hidden lg:block absolute top-[60%] left-[15%] right-[15%] h-px bg-[#063326]/5 z-0"></div>
    </div>
  );
};
