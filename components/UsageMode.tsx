      ...
      <div className="container mx-auto px-4">
        ...
        <div ref={reveal.ref} className="grid grid-cols-1 md:grid-cols-3 gap-8 lg:gap-12">
          {steps.map((item, idx) => (
            <div 
              key={idx} 
              className={`flex flex-col items-center text-center animeTexto bg-white p-12 rounded-xl border border-gray-100 hover:border-[#E3C08D]/30 group transition-all duration-500 hover:shadow-xl ${reveal.className}`}
              style={{ transitionDelay: `${idx * 200}ms` }}
            >
              <div className="mb-8 text-[#E3C08D] p-4 bg-[#FCF9F4] rounded-full group-hover:bg-[#E3C08D] group-hover:text-white transition-colors duration-500">
                {item.icon}
              </div>
              <h3 className="text-xl font-bold mb-6 text-gray-800 uppercase tracking-wider">{item.title}</h3>
              <p className="text-gray-500 text-sm md:text-base leading-relaxed font-light transition-colors" dangerouslySetInnerHTML={{ __html: item.description }}>
              </p>
            </div>
          ))}
        </div>
        ...
        <div className="mt-24 text-center">
          <p className="text-[11px] uppercase tracking-[0.2em] font-bold text-[#E3C08D]">
